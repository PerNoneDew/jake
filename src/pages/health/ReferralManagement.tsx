...
export default function ReferralManagement() {
  ...
  const canManage = isAdmin || isStaff || isOfficer;
  const patients = users.filter((u) => ['student', 'staff', 'faculty', 'employee'].includes(u.role));
  const displayReferrals = isRegularUser ? referrals.filter((r) => r.patientId === currentUser.id) : referrals;
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
        <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {canManage && <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors"><Plus size={15} /> New Referral</button>}
          </div>
        </div>
