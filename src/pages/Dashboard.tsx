import {
  Users, FileText, ClipboardList, AlertTriangle, CheckCircle, Clock, TrendingUp,
  GraduationCap, Briefcase, Stethoscope, Smile, HeartPulse, Pill, Share2, RefreshCw,
  CalendarClock, Activity, PackageCheck,
  ArrowRight, PackageX,
} from 'lucide-react';
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const isOfficer = role === 'health_officer';
  ...
  const todayDental = todayRequests.filter((r) => r.type === 'dental');
  const todayMedicine = todayRequests.filter((r) => r.type === 'medicine');
  ...
  const quickActions: { label: string; icon: typeof Users; page: Page; color: string }[] = [
    { label: 'New Treatment', icon: Stethoscope, page: 'daily-treatment', color: 'bg-teal-50 text-teal-600 hover:bg-teal-100' },
    { label: 'Dental Examination', icon: Smile, page: 'dental', color: 'bg-sky-50 text-sky-600 hover:bg-sky-100' },
    { label: 'Physical Examination', icon: HeartPulse, page: 'physical-examination', color: 'bg-rose-50 text-rose-600 hover:bg-rose-100' },
    { label: 'Medicine Issuance', icon: Pill, page: 'medical-issuance', color: 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' },
    { label: 'New Referral', icon: Share2, page: 'referral', color: 'bg-violet-50 text-violet-600 hover:bg-violet-100' },
    { label: 'Add Students', icon: GraduationCap, page: 'students', color: 'bg-amber-50 text-amber-600 hover:bg-amber-100' },
    { label: 'Add Employee', icon: Briefcase, page: 'employees', color: 'bg-slate-100 text-slate-600 hover:bg-slate-200' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <DashStat title="Dental Exams Today" value={todayDental.length} subtitle="Dental examinations" icon={Smile} color="amber" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Physical Exams Today" value={todayMedical.length} subtitle="Physical checkups" icon={HeartPulse} color="rose" onClick={() => onNavigate('daily-treatment')} />
          </div>

          {/* 2.7-2.11: Medicine & Inventory */}
      ...
      {isOfficer && (
        <>
          {/* 2.23: Patient Summary */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Total Patients" value={healthRecords.filter((r) => !r.archived).length} subtitle="Active records" icon={Users} color="teal" onClick={() => onNavigate('health-records')} />
            <DashStat title="Today's Visits" value={todayRequests.length} subtitle="Patient visits today" icon={Stethoscope} color="sky" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Pending Reviews" value={pendingRequests.length} subtitle="Awaiting review" icon={ClipboardList} color="amber" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Approved Today" value={todayRequests.filter((r) => r.status === 'approved' || r.status === 'released').length} subtitle="Completed today" icon={CheckCircle} color="emerald" onClick={() => onNavigate('daily-treatment')} />
          </div>

          {/* 2.24: Dispensary Activities */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Medicines Issued Today" value={todayDispensing.length} subtitle="Dispensed today" icon={Pill} color="teal" onClick={() => onNavigate('medicines')} />
            <DashStat title="Total Dispensing" value={dispensingHistory.length} subtitle="All-time records" icon={PackageCheck} color="sky" onClick={() => onNavigate('medicines')} />
            <DashStat title="Pending Referrals" value={pendingReferrals.length} subtitle="Forwarded patients" icon={Share2} color="violet" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Pending Follow-ups" value={pendingFollowups.length} subtitle="In progress" icon={RefreshCw} color="amber" onClick={() => onNavigate('daily-treatment')} />
          </div>

          {/* 2.25: Medicine Alerts */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Low-Stock Items" value={lowStockItems.length} subtitle="Need restocking" icon={AlertTriangle} color="amber" onClick={() => onNavigate('medicines')} />
            <DashStat title="Out-of-Stock" value={outOfStockItems.length} subtitle="Zero quantity" icon={PackageX} color="rose" onClick={() => onNavigate('medicines')} />
            <DashStat title="Near-Expiry" value={nearExpiryItems.length} subtitle="Within 90 days" icon={Clock} color="amber" onClick={() => onNavigate('medicines')} />
            <DashStat title="Expired" value={expiredItems.length} subtitle="Past expiry date" icon={AlertTriangle} color="rose" onClick={() => onNavigate('medicines')} />
          </div>

          {/* 2.26: Statistics */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h3 className="font-semibold text-slate-800 text-sm mb-4">Service Statistics</h3>
              <div className="space-y-3">
                {(['medical', 'dental', 'medicine'] as const).map((type) => {
                  const count = requests.filter((r) => r.type === type).length;
                  const max = Math.max(requests.length, 1);
                  return (
                    <div key={type}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-slate-600 font-medium capitalize">{type}</span>
                        <span className="font-bold text-slate-800">{count}</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2">
                        <div className="bg-teal-400 h-2 rounded-full transition-all" style={{ width: `${(count / max) * 100}%` }} />
                      </div>
                    </div>
                  );
                })}
            ...
            <AlertPanel
              ...
              icon={AlertTriangle}
              accent="amber"
              items={lowStockItems.map((m) => ({ id: m.id, name: m.name, meta: `${m.quantity} ${m.unit} left`, badge: 'Low', badgeColor: 'amber' as const }))}
              emptyMessage="All items sufficiently stocked."
              onNavigate={() => onNavigate('medicines')}
            />
            ...
            <AlertPanel
              ...
              icon={Clock}
              accent="sky"
              items={pendingRequests.slice(0, 5).map((r) => ({ id: r.id, name: r.userName, meta: `${r.type} · ${r.submittedAt}`, badge: r.status, badgeColor: 'amber' as const }))}
              emptyMessage="No pending reviews."
              onNavigate={() => onNavigate('daily-treatment')}
            />
          </div>
        </>
      )}
      ...
      {isStaff && (
        <>
          {/* 2.17: Dispensary Activities */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Dispensed Today" value={todayDispensing.length} subtitle="Today's dispensing" icon={Pill} color="teal" onClick={() => onNavigate('medicines')} />
            <DashStat title="Total Dispensing" value={dispensingHistory.length} subtitle="All-time records" icon={PackageCheck} color="sky" onClick={() => onNavigate('medicines')} />
            <DashStat title="Today's Visits" value={todayRequests.length} subtitle="Patient visits" icon={Stethoscope} color="violet" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Pending Requests" value={pendingRequests.length} subtitle="Awaiting processing" icon={ClipboardList} color="amber" onClick={() => onNavigate('daily-treatment')} />
          </div>

          {/* 2.18: Patient Summary */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Total Patients" value={healthRecords.filter((r) => !r.archived).length} subtitle="Active records" icon={Users} color="teal" onClick={() => onNavigate('health-records')} />
            <DashStat title="Today's Treatments" value={todayMedical.length} subtitle="Medical today" icon={Activity} color="emerald" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Dental Today" value={todayDental.length} subtitle="Dental exams today" icon={Smile} color="amber" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Medicine Requests" value={todayMedicine.length} subtitle="Medicine today" icon={Pill} color="sky" onClick={() => onNavigate('daily-treatment')} />
          </div>

          {/* 2.19: Medicine Alerts */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <DashStat title="Low-Stock" value={lowStockItems.length} subtitle="Need restocking" icon={AlertTriangle} color="amber" onClick={() => onNavigate('medicines')} />
            <DashStat title="Out-of-Stock" value={outOfStockItems.length} subtitle="Zero quantity" icon={PackageX} color="rose" onClick={() => onNavigate('medicines')} />
            <DashStat title="Near-Expiry" value={nearExpiryItems.length} subtitle="Within 90 days" icon={Clock} color="amber" onClick={() => onNavigate('medicines')} />
            <DashStat title="Expired" value={expiredItems.length} subtitle="Past expiry" icon={AlertTriangle} color="rose" onClick={() => onNavigate('medicines')} />
          </div>

          {/* 2.20-2.22: Referrals, Follow-ups, Appointments */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <DashStat title="Pending Referrals" value={pendingReferrals.length} subtitle="Awaiting external care" icon={Share2} color="sky" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Pending Follow-ups" value={pendingFollowups.length} subtitle="In progress" icon={RefreshCw} color="amber" onClick={() => onNavigate('daily-treatment')} />
            <DashStat title="Upcoming Appointments" value={upcomingAppointments.length} subtitle="Approved & scheduled" icon={CalendarClock} color="teal" onClick={() => onNavigate('daily-treatment')} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <AlertPanel
              ...
              icon={Pill}
              accent="teal"
              items={dispensingHistory.slice(0, 5).map((d) => ({ id: d.id, name: d.medicineName, meta: `${d.patientName} · ${d.quantity} ${d.unit} · ${d.dispensedAt}`, badge: d.dispensedAt, badgeColor: 'slate' as const }))}
              emptyMessage="No dispensing records."
              onNavigate={() => onNavigate('medicines')}
            />
            <AlertPanel
              ...
              accent="amber"
              items={[
                ...lowStockItems.slice(0, 3).map((m) => ({ id: m.id, name: m.name, meta: `${m.quantity} ${m.unit} left`, badge: 'Low', badgeColor: 'amber' as const })),
                ...outOfStockItems.slice(0, 2).map((m) => ({ id: m.id, name: m.name, meta: 'Out of stock', badge: 'Out', badgeColor: 'rose' as const })),
              ]}
              emptyMessage="All items sufficiently stocked."
              onNavigate={() => onNavigate('medicines')}
            />
