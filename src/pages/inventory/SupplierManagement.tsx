...
const emptyForm: FormData = { name: '', contactPerson: '', email: '', phone: '', address: '', category: 'General', status: 'active' };
...
export default function SupplierManagement() {
  ...
  const canManage = isOfficer || isStaff;
  ...
  const openEdit = (s: Supplier) => {
    setEditItem(s);
    setForm({ name: s.name, contactPerson: s.contactPerson ?? '', email: s.email ?? '', phone: s.phone ?? '', address: s.address ?? '', category: s.category, status: s.status });
    setShowForm(true);
  };
