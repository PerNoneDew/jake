...
export default function PurchaseManagement() {
  ...
  const { purchases, persistPurchase, removePurchase, suppliers } = useData();
    ...
    await runWithFeedback(
      async () => {
        if (editItem) {
          ...
          await persistPurchase({ ...form, totalCost, id: `pur${Date.now()}`, createdAt: new Date().toISOString().split('T')[0] });
        }
      },
