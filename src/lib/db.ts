...
const supToRow = (s: MedicalSupply): MedicalSupplyRow => ({
  ...
  expiry_date: s.expiryDate, supplier: s.supplier, last_updated: s.lastUpdated, primary_key_date: s.primaryKeyDate ?? null,
});
...
type SupplierRow = {
  ...
  address: string | null; category: string; status: string; created_at: string;
};
const supplierToRow = (s: Supplier): SupplierRow => ({
  ...
  address: s.address ?? null, category: s.category, status: s.status, created_at: s.createdAt,
});
const rowToSupplier = (r: SupplierRow): Supplier => ({
  ...
  address: r.address ?? undefined, category: r.category, status: r.status as Supplier['status'], createdAt: r.created_at,
});
